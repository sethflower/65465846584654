...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _handleRegistration() async {
    ...
    try {
      ...
      setState(() {
        _registerSuccess = false;
        _registerMessage =
            'Не вдалося відправити заявку. Спробуйте пізніше.';
      });
    } finally {
      if (mounted) {
  ...
  Future<void> _openAdminPanel() async {
    ...
    final password = await showDialog<String?>(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text('Вхід до панелі адміністратора'),
          content: TextField(
            controller: controller,
            obscureText: true,
            autofocus: true,
            decoration: const InputDecoration(labelText: 'Пароль адміністратора'),
          ),
          actions: [
            TextButton(
                  ...
                  SizedBox(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 18),
                        Text('TrackingApp', textAlign: isCompact ? TextAlign.center : TextAlign.left, style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.w900, color: AppColors.navy)),
                        const SizedBox(height: 10),
                        Text('Операционный вход для сканирования BoxID и ТТН', textAlign: isCompact ? TextAlign.center : TextAlign.left, style: Theme.of(context).textTheme.bodyLarge?.copyWith(color: AppColors.slate, height: 1.45)),
                      ],
                    ),
                  ),
                  SizedBox(
